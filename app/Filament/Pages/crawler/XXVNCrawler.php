...
namespace App\Filament\Pages\crawler;
...
class XXVNCrawler extends Page
{
    protected static ?string $navigationIcon = 'heroicon-o-cloud';
    protected static string $view = 'filament.pages.fetch-movies';
    protected static ?string $navigationGroup = 'Cào Phim';
    protected static ?string $title = 'XXVN Crawler';
    protected static ?string $navigationLabel = 'XXVN Crawler';

    public $api_url = 'https://xxvnapi.com/api/phim-moi-cap-nhat';
    ...
    private function fetchSingleMovies()
    {
        ...
        $responses = Http::pool(
            fn($pool) => array_map(fn($url) => $pool->get($url), $urls)
        );
        ...
        foreach ($responses as $index => $response) {
            if ($response->successful()) {
                ...
                if (!empty($data)) {
                    $movieGenres = isset($data['categories']) && is_array($data['categories'])
                        ? array_column($data['categories'], 'name')
                        : [];
                    $movieCountry = isset($data['country']['slug']) ? $data['country']['slug'] : '';
                    $movieType = isset($data['type']) ? $data['type'] : 'single';
                    ...
                    $shouldSkip = (
                        ...
                        (isset($this->selectedType) && in_array($movieType, $this->selectedType))
                    );
                    ...
                    if (!$shouldSkip) {
                        ...
                        if (!empty($data['slug'])) {
                            $this->movielink[] = "https://xxvnapi.com/api/phim/{$data['slug']}";
                        }
                    }
                }
            } else {
    ...
    private function fetchAllMovies()
    {
        ...
        $this->movies = [];
        $batchSize = 25;
        $pages = range($this->page_to, $this->page_from);
        $responses = [];
        ...
        foreach (array_chunk($pages, $batchSize) as $batch) {
            $batchResponses = Http::pool(
                fn($pool) =>
                array_map(
                    fn($page) => $pool->timeout(20)->retry(3, 5000)->get($this->api_url, [
                        'page' => $page,
                        'limit' => $this->limit,
                    ]),
                    $batch
        ...
        foreach ($responses as $index => $response) {
            if ($response->successful()) {
                ...
                $filteredMovies = array_filter($data, function ($movie) {
                    $movieGenres = isset($movie['categories']) && is_array($movie['categories'])
                        ? array_column($movie['categories'], 'name')
                        : [];
                    $movieCountry = isset($movie['country']['slug']) ? $movie['country']['slug'] : '';
                    $movieType = isset($movie['type']) ? $movie['type'] : 'single';
                    return !(
                        (isset($this->selectedGenre) && array_intersect($movieGenres, $this->selectedGenre)) ||
                        (isset($this->selectedCountry) && in_array($movieCountry, $this->selectedCountry)) ||
                        (isset($this->selectedType) && in_array($movieType, $this->selectedType))
                ...
                $slugs = array_map(fn($slug) => "https://xxvnapi.com/api/phim/{$slug}", array_column($filteredMovies, 'slug'));
    ...
    private function fetchMovieDetails(): void
    {
        ...
        $movieUrls = array_map(fn($movie) => "https://xxvnapi.com/api/phim/{$movie['slug']}", $this->movies);
    ...
    private function processMovieResponse($movieResponse, $index): void
    {
        ...
        if ($movieResponse->successful() && $movie) {
            ...
            $this->successMovies[] = "https://xxvnapi.com/api/phim/" . $movieDetails['slug'];
        } else {
            $this->failedMovies[] = "https://xxvnapi.com/api/phim/" . $movie['slug'] ?? 'Không có tên';
        }
    }
    ...
    private function proccessMovieDetail($movieDetails)
    {
        $movieModel = Movie::updateOrCreate(
            ...
            [
                ...
                'thumb_url' => $this->saveImage($movieDetails['thumb_url'] ?? null, $movieDetails['slug'] . '_thumb' ,$this->thumbwidth, $this->thumbheight),
                ...
                'languages' => $movieDetails['lang'] ?? 'VietSub',
    ...
    private function syncRegion(array $regionData = [], $movie)
    {
    ...
    private function syncEpisode(array $episodes = [], $movie)
    {
        foreach ($episodes as $server) {
            if (!isset($server['server_name']) || !isset($server['server_data'])) {
                continue;
            }
            foreach ($server['server_data'] as $episode) {
                if (!isset($episode['name'], $episode['slug'], $episode['link'])) {
                    continue;
                }
